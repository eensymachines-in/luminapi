...
type IScheds interface {
	RelayStates() []scheduling.JSONRelayState
...
func (dr *DevReg) RelayStates() []scheduling.JSONRelayState {
	return dr.Scheds
}
...
func (dreg *DevReg) QGetLogs(lvl string, q func(pipe []bson.M) error) error {
	...
	return q([]bson.M{match_serial, stage_unwind, stage_project, sort_time, match_lvl, grp_logs})
}
