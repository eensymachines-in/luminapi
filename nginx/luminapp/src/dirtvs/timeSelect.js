    ...
    angular.module("luminapp")
        .directive("timePick", function () {
            return {
                ...
                replace: true,
                scope: {
                    tm: "=", // the time objet that this picker writes to
                    title: "@", // title that appears 
                    desc: "@",
                    title: "@"
                },
                templateUrl: "/templates/time-pick.html",
                controller: function ($scope) {
                    $scope.hrOptions = Array.from(Array.from(Array(13).keys()).slice(1), function (el) {
                        return el.toString().padStart(2, 0)
                    }); //array of hours 1-12 
                    $scope.mnOptions = Array.from(Array.from(Array(60).keys()), function (el) {
                        return el.toString().padStart(2, 0)
                    });// array of minutes 0-59 
                    $scope.mrOptions = ["AM", "PM"]; // Meridiem options
                    if (!$scope.tm) {
                        $scope.tm = {
                            hr: $scope.hrOptions[0],
                            mn: $scope.mnOptions[0],
                            mr: $scope.mrOptions[0]
                        }
                    } // if the tm object is not assigned then we choose the first from each of the options 
                }
            }
        })
